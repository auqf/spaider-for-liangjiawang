...
with open('./land_agent_url_txt') as fd:
    while True:
        ...
        a=MyThread(start_time,GetHtml(url.replace(".html","/news.html")))
        b=MyThread(price_update_time,GetHtml(url.replace(".html","/ht.html")))
        c=MyThread(detail_info,GetHtml(url.replace(".html","/details.html")))
        d=MyThread(around_info,GetHtml(url.replace(".html","/su.html")))
        threads = [a, b, c, d]
        ...
        print >> land_agent_detail_info_f,"%s*%s*%s*%s*%s"%(url.strip("\n"),a.get_result(),b.get_result(),c.get_result(),d.get_result())
        #print >>land_agent_detail_info_f,"%s*%s*%s*%s*%s"%(url.strip("\n"),a,b,c,d)
...
with open('./house_type_url_txt') as fd:
    while True:
        ...
        a=MyThread(house_type_info,GetHtml(url))
        b=MyThread(house_properties,GetHtml(url))
        threads = [a, b]
        for _ in threads:
            _.setDaemon(True)
            _.start()
